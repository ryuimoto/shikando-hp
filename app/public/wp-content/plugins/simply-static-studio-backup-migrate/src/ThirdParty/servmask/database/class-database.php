...
abstract class Database {
	...
	public function export( $file_name, &$query_offset = 0, &$table_index = 0, &$table_offset = 0, &$table_rows = 0 ) {
		// Set file handler
		$file_handler = @fopen( $file_name, 'cb' );
		...
		if ( fseek( $file_handler, $query_offset ) !== -1 ) {
			...
			// Write headers
			if ( $query_offset === 0 ) {
				Helper::write( $file_handler, $this->get_header() );
			}
			...
			for ( ; $table_index < count( $tables ); ) {
				...
				if ( in_array( $table_name, $views ) ) {
					...
					if ( $table_offset === 0 ) {
						...
						Helper::write( $file_handler, $drop_view );
						...
						Helper::write( $file_handler, $create_view );

						// Write end of statement
						Helper::write( $file_handler, ";\n\n" );
					}
					...
					if ( $table_offset === 0 ) {
						...
						Helper::write( $file_handler, $drop_table );
						...
						Helper::write( $file_handler, $create_table );
					...
					do {
						...
						if ( $primary_keys ) {
							...
							if ( defined( 'sssbm_DISABLE_LATE_ROW_LOOKUPS' ) ) {
								$query = sprintf( 'SELECT %s FROM `%s` AS t1 WHERE %s ORDER BY %s LIMIT %d, %d', $select_columns, $table_name, $table_where, $table_keys, $table_offset, 1000 );
							} else {
								$query = sprintf( 'SELECT %s FROM `%s` AS t1 JOIN (SELECT %s FROM `%s` WHERE %s ORDER BY %s LIMIT %d, %d) AS t2 USING (%s)', $select_columns, $table_name, $table_keys, $table_name, $table_where, $table_keys, $table_offset, 1000, $table_keys );
							}
						} else {
							...
							// Set query with offset and rows count
							$query = sprintf( 'SELECT %s FROM `%s` WHERE %s ORDER BY %s LIMIT %d, %d', $select_columns, $table_name, $table_where, $table_keys, $table_offset, 1000 );
						}
						...
						if ( $num_rows = $this->num_rows( $result ) ) {
							...
							while ( $row = $this->fetch_assoc( $result ) ) {
								...
								if ( $table_offset % 1000 === 0 ) {
									Helper::write( $file_handler, "START TRANSACTION;\n" );
								}
								...
								Helper::write( $file_handler, $table_insert );
								...
								if ( $table_offset % 1000 === 0 ) {
									Helper::write( $file_handler, "COMMIT;\n" );
								}
							}
						} else {
	...
	protected function replace_visual_composer_values_callback( $matches ) {
		...
		if ( Database_Utility::base64_validate( $matches[1] ) ) {

			// Decode base64 characters
			$matches[1] = Database_Utility::base64_decode( $matches[1] );

			// Replace values
			$matches[1] = Database_Utility::replace_values( $matches[1], $this->get_old_replace_values(), $this->get_new_replace_values() );

			// Encode base64 characters
			$matches[1] = Database_Utility::base64_encode( $matches[1] );
		}
	...
	protected function replace_oxygen_builder_values_callback( $matches ) {
		// Validate base64 data
		if ( Database_Utility::base64_validate( $matches[2] ) ) {

			// Decode base64 characters
			$matches[2] = Database_Utility::base64_decode( $matches[2] );

			// Replace values
			$matches[2] = Database_Utility::replace_values( $matches[2], $this->get_old_replace_values(), $this->get_new_replace_values() );

			// Encode base64 characters
			$matches[2] = Database_Utility::base64_encode( $matches[2] );
		}
	...
	protected function replace_base64_values_callback( $matches ) {
		...
		if ( Database_Utility::base64_validate( $matches[1] ) ) {
			...
			// Replace serialized values
			$matches[1] = Database_Utility::replace_serialized_values( $matches[1], $this->get_old_replace_values(), $this->get_new_replace_values() );
	...
	protected function replace_table_values_callback( $matches ) {
		...
		$matches[1] = Database_Utility::unescape_mysql( $matches[1] );

		// Replace serialized values
		$matches[1] = Database_Utility::replace_serialized_values( $matches[1], $this->get_old_replace_values(), $this->get_new_replace_values() );

		// Escape MySQL special characters
		$matches[1] = Database_Utility::escape_mysql( $matches[1] );
	...
	protected function replace_raw_values( $input ) {
		return Database_Utility::replace_values( $input, $this->get_old_replace_raw_values(), $this->get_new_replace_raw_values() );
	}
	...
	protected function get_header() {
		...
		$header = sprintf(
			"-- Simply Static Studio Migration SQL Dump\n" .
